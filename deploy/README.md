...
Search the directory for `viralsangani` — these are the GitHub repo path. Set
them to your fork URL (e.g. `viralsangani/mini-quiz`).

```bash
grep -rl viralsangani deploy/ | xargs sed -i '' 's|viralsangani|viralsangani|g'
```
