...
Using our [SDKs](/docs/libraries.md) or [API](/docs/api/capture.md) to [capture events](/docs/getting-started/send-events.md), [identify users](/docs/getting-started/identify-users.md), create groups, and add properties adds data to these tables, that you can query using the [SQL editor](/docs/data-warehouse/query.md).
...
| properties | Object containing all properties of the event (stored as a String). You can see the default properties in [our data docs](/docs/data/events.md#default-properties). |
...
The users behind your events are known as [persons](/docs/data/persons.md) in PostHog.
...
| last_seen_at | Date and time of the person's most recent event (ISO 8601 format), rounded to the hour. See [skipping last_seen_at updates](/docs/product-analytics/person-properties.md#skipping-last_seen_at-updates). |
...
Beyond storing demographic data about your users, the `persons` table is powerful as it can connect to many other tables. For example, the `persons` table can be queried through the `events` using the `person` field like `SELECT person.properties.$initial_browser FROM events`. It can also be [joined to external sources](/docs/data-warehouse/join.md) to create extended person properties.

### `groups`

Groups enable you to aggregate events based on an entity like an organization, project, or team if you have set up [group analytics](/docs/product-analytics/group-analytics.md).
...
If you are using PostHog's [snippet](/docs/getting-started/install?tab=snippet.md) or [JavaScript Web SDK](/docs/libraries/js.md), PostHog automatically captures session data which you can query using the `sessions` table.
...
The `query_log` table provides access to query execution metadata and performance metrics. You can learn more about it in the [query log data docs](/docs/data/query-log.md).
...
| [View additional query log fields](/docs/data/query-log.md#table-schema) | See [full list](/docs/data/query-log.md#table-schema) of query log properties |
