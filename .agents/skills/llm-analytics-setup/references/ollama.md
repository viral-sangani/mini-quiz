...
1.  1
    ...
    **Note:** Make sure Ollama is running locally before making API calls. You can start it with `ollama serve`.
...
3.  3
    ...
    We call Ollama through the OpenAI client and generate a response. We'll use PostHog's OpenAI provider to capture all the details of the call. Initialize PostHog with your PostHog project token and host from [your project settings](https://app.posthog.com/settings/project), then pass the PostHog client along with the Ollama config (the base URL) to our OpenAI wrapper.
...
4.  4
    ...
    Now, when you call Ollama with the OpenAI SDK, PostHog automatically captures an `$ai_generation` event. You can also capture or modify additional properties with the distinct ID, trace ID, properties, groups, and privacy mode parameters.
